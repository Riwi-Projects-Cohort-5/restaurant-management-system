...
function CartPanel() {
  ...
  var html = '<div class="pos-cart bg-white border border-brand-300 rounded-xl shadow-sm p-5 h-full flex flex-col">';
...
function setupCartEvents() {
  ...
  window.addEventListener('cart:add', function (e) {
    ...
    if (existing) {
      ...
      cartItems.push({ id: item.id, name: item.name, price: item.price, qty: 1, emoji: item.emoji });
    }
    refreshCart();
  });
  ...
  document.addEventListener('click', function (e) {
    if (e.target.id === 'cart-send-kitchen') {
      ...
      var maxId = allOrders.reduce(function (m, o) { return Math.max(m, o.id); }, 1034);
      var newOrder = {
        id: maxId + 1,
        table: Math.floor(Math.random() * 12) + 1,
        items: cartItems.map(function (c) { return { name: c.name, qty: c.qty, price: c.price }; }),
        total: 0,
        status: 'new',
        time: 'Just now',
        note: null,
    ...
    if (e.target.id === 'cart-save-draft') {
      if (cartItems.length === 0) return;
      var maxId2 = allOrders.reduce(function (m, o) { return Math.max(m, o.id); }, 1034);
      var draft = {
        id: maxId2 + 1,
        table: 0,
        items: cartItems.map(function (c) { return { name: c.name, qty: c.qty, price: c.price }; }),
