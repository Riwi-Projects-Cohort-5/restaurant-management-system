...
function Skeleton(opts = {}) {
  ...
  if (variant === "circle") {
    ...
    return `<div class="rounded-full bg-neutral-200 animate-pulse ${cls}" style="width:${size}px;height:${size}px;" aria-hidden="true"></div>`;
  }

  if (variant === "rect") {
    const w = opts.width || "100%";
    const h = opts.height || 200;
    return `<div class="rounded-lg bg-neutral-200 animate-pulse ${cls}" style="width:${w};height:${h}px;" aria-hidden="true"></div>`;
  }
  ...
  for (let i = 0; i < lines; i++) {
    ...
    items.push(
      `<div class="h-4 rounded bg-neutral-200 animate-pulse ${cls}" style="width:${lineW};" aria-hidden="true"></div>`
    );
  }
