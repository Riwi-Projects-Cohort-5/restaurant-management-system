...
function StatusStepper(opts) {
  ...
  if (cancelled) {
    ...
    html += '<span class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-error-100 text-error-700 font-semibold text-sm">';
  ...
  LIFECYCLE.forEach(function (step, i) {
    ...
    html += '<div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ' + dotClass + '">';
    if (isDone) {
    ...
    html += '</div>';
    html += '<span class="text-[10px] font-semibold mt-1 text-center capitalize whitespace-nowrap ' + (isCurrent ? 'text-primary-700' : 'text-brand-500') + '">' + step + '</span>';
    html += '</div>';

    if (isConnector) {
      html += '<div class="h-0.5 flex-1 mx-2 mt-[-14px] ' + (i < ci ? 'bg-success-500' : 'bg-brand-200') + '"></div>';
    }
