...
function statusBadge(status, size) {
  ...
  var sizeClasses = size === 'lg'
    ? 'px-3 py-1 text-sm'
    : 'px-2.5 py-0.5 text-xs';
  var dotSize = size === 'lg' ? 'h-2 w-2' : 'h-1.5 w-1.5';
  return '<span class="inline-flex items-center gap-1.5 rounded-full font-semibold ' + sizeClasses + ' ' + colors.bg + ' ' + colors.text + '">' +
    '<span class="' + dotSize + ' rounded-full ' + colors.dot + '"></span>' +
    label + '</span>';
}
...
function getFiltered() {
  ...
  if (activeFilter !== 'all') {
    filtered = filtered.filter(function (r) { return r.status === activeFilter; });
  }
...
function renderList(el) {
  ...
  ['pending', 'confirmed', 'cancelled', 'completed'].forEach(function (s) {
    counts[s] = allRes.filter(function (r) { return r.status === s; }).length;
  });
  ...
  html += '<p class="text-sm text-secondary-500 mt-0.5">' + reservations.length + ' reservation' + (reservations.length !== 1 ? 's' : '') + '</p></div>';
  html += '<button data-action="new-reservation" class="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white border-0 cursor-pointer transition-colors"><i data-lucide="plus" class="w-4 h-4"></i> New Reservation</button>';
  html += '</div>';
  ...
  html += '<input type="text" id="res-search" value="' + searchQuery + '" placeholder="Search by code, guest, or phone..." class="flex-1 text-sm text-neutral-900 outline-none border-none bg-transparent placeholder:text-secondary-400" />';
  if (searchQuery) {
  ...
  html += '</div>';
  html += '<input type="date" id="res-date-filter" class="border border-brand-200 rounded-lg px-3 py-2 text-sm text-neutral-700 bg-white cursor-pointer" />';
  html += '</div>';
  html += '</div>';
  ...
  if (reservations.length === 0) {
    ...
    html += '<p class="text-sm text-secondary-400">' + (searchQuery ? 'No reservations match your search' : 'No reservations found') + '</p>';
    if (searchQuery) {
      html += '<button data-action="clear-search" class="text-sm text-primary-600 hover:text-primary-700 font-semibold cursor-pointer bg-transparent border-none">Clear search</button>';
    }
    html += '</div></td></tr>';
  } else {
    reservations.forEach(function (r) {
      html += '<tr class="border-b border-brand-100 hover:bg-brand-50/50 transition-colors" data-reservation-id="' + r.id + '">';
      ...
      if (r.guestPhone) html += '<div class="text-xs text-secondary-400">' + r.guestPhone + '</div>';
      html += '</td>';
      html += '<td class="px-5 py-3.5 text-sm text-neutral-700">' + r.date + '</td>';
      html += '<td class="px-5 py-3.5 text-sm text-neutral-700">' + r.time + '</td>';
      html += '<td class="px-5 py-3.5 text-sm text-neutral-700 text-center">' + r.partySize + '</td>';
      html += '<td class="px-5 py-3.5 text-sm text-neutral-700 text-center">' + (r.tableNumber || '\u2014') + '</td>';
      html += '<td class="px-5 py-3.5">' + statusBadge(r.status) + '</td>';
      html += '<td class="px-5 py-3.5"><button data-action="view-detail" data-id="' + r.id + '" class="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-lg bg-transparent text-brand-500 hover:bg-brand-100 border border-brand-300 cursor-pointer transition-colors"><i data-lucide="eye" class="w-3.5 h-3.5"></i> View</button></td>';
      html += '</tr>';
    });
  }
...
function renderDetail(el) {
  var r = reservationStore.getState().reservations.find(function (x) { return x.id === selectedId; });
  if (!r) { subView = 'list'; selectedId = null; renderList(el); return; }
  ...
  html += '<h2 class="text-xl font-semibold text-primary-700 font-display">Reservation ' + r.code + '</h2>';
  html += '</div>';
  html += '<div class="flex items-center gap-3">' + statusBadge(r.status, 'lg');
  if (r.status === 'pending' || r.status === 'confirmed') {
    html += '<div class="flex gap-2">';
    if (r.status === 'pending') {
      html += '<button data-action="confirm" data-id="' + r.id + '" class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-success-600 hover:bg-success-700 text-white border-0 cursor-pointer transition-colors"><i data-lucide="check" class="w-3.5 h-3.5"></i> Confirm</button>';
    }
    if (r.status === 'confirmed') {
      html += '<button data-action="complete" data-id="' + r.id + '" class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white border-0 cursor-pointer transition-colors"><i data-lucide="check-circle" class="w-3.5 h-3.5"></i> Complete</button>';
    }
    html += '<button data-action="cancel" data-id="' + r.id + '" class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-transparent text-error-600 hover:bg-error-50 border border-error-300 cursor-pointer transition-colors"><i data-lucide="x" class="w-3.5 h-3.5"></i> Cancel</button>';
    html += '</div>';
  }
  html += '</div></div>';

  html += '<div class="grid grid-cols-4 gap-4">';
  html += renderInfoCard('Guest', '<span class="text-lg font-bold text-brand-900">' + r.guestName + '</span>' + (r.guestPhone ? '<span class="block text-sm text-secondary-500 mt-0.5">' + r.guestPhone + '</span>' : ''));
  html += renderInfoCard('Date & Time', '<span class="text-lg font-bold text-brand-900">' + r.date + '</span><span class="block text-sm text-secondary-500 mt-0.5">' + r.time + '</span>');
  html += renderInfoCard('Party', '<span class="text-lg font-bold text-brand-900">' + r.partySize + '</span><span class="block text-sm text-secondary-500 mt-0.5">guests</span>');
  html += renderInfoCard('Table', '<span class="text-lg font-bold text-brand-900">' + (r.tableNumber || '\u2014') + '</span>');
  html += '</div>';

  html += '<div class="bg-white border border-brand-300 rounded-xl shadow-[0_2px_6px_rgba(114,49,23,0.08)] overflow-hidden">';
  ...
  html += '<div class="flex items-center gap-3 text-sm"><span class="font-semibold text-secondary-500 w-28">Created</span><span class="text-neutral-700">' + new Date(r.createdAt).toLocaleDateString() + ' at ' + new Date(r.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + '</span></div>';
  html += '<div class="flex items-center gap-3 text-sm"><span class="font-semibold text-secondary-500 w-28">Status</span>' + statusBadge(r.status) + '</div>';
  if (r.notes) {
    html += '<div class="flex items-start gap-3 text-sm"><span class="font-semibold text-secondary-500 w-28 shrink-0">Notes</span><span class="text-neutral-700 italic">' + r.notes + '</span></div>';
  }
  html += '</div></div>';
...
function renderNewReservationForm(el) {
  ...
  html += '<button data-action="back" class="px-4 py-2 text-sm font-semibold rounded-lg bg-transparent text-brand-600 hover:bg-brand-50 border border-brand-300 cursor-pointer transition-colors">Cancel</button>';
  html += '<button data-action="save-reservation" class="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-primary-600 hover:bg-primary-700 text-white border-0 cursor-pointer transition-colors"><i data-lucide="check" class="w-4 h-4"></i> Save Reservation</button>';
  html += '</div>';
  html += '</div>';
...
function renderFormField(id, label, type, placeholder, required) {
  return '<label class="flex flex-col gap-1 text-xs font-semibold text-secondary-600">' + label +
  ...
  el.addEventListener('click', function (e) {
    ...
    if (saveBtn) {
      ...
      if (!name.trim() || !date || !time) {
        var firstEmpty = !name.trim() ? document.getElementById('res-guest-name')
          : !date ? document.getElementById('res-date')
          : document.getElementById('res-time');
        if (firstEmpty) {
          firstEmpty.classList.add('border-error-400');
          firstEmpty.focus();
          setTimeout(function () { firstEmpty.classList.remove('border-error-400'); }, 2000);
        }
        return;
      }
