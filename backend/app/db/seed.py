...
def seed_database(db: Session) -> None:
    ...
    tables = [
        Table(
            id=uuid.uuid4(), number=1, capacity=2,
            status=TableStatus.AVAILABLE, location="Interior",
        ),
        Table(
            id=uuid.uuid4(), number=2, capacity=4,
            status=TableStatus.AVAILABLE, location="Interior",
        ),
        Table(
            id=uuid.uuid4(), number=3, capacity=6,
            status=TableStatus.AVAILABLE, location="Terraza",
        ),
        Table(
            id=uuid.uuid4(), number=4, capacity=8,
            status=TableStatus.AVAILABLE, location="Terraza",
        ),
    ]
    db.add_all(tables)
    ...
    menu_items = [
        MenuItem(
            id=uuid.uuid4(), name="Nachos", description="Nachos con guacamola",
            price=8.50, category_id=cat_map["Entradas"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Ensalada Cesar", description="Ensalada con pollo",
            price=10.00, category_id=cat_map["Entradas"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Ribeye",
            description="Ribeye 300g con guarnicion",
            price=25.00, category_id=cat_map["Platos Fuertes"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Salmon",
            description="Salmon al horno con vegetales",
            price=22.00, category_id=cat_map["Platos Fuertes"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Coca-Cola", description="355ml",
            price=2.50, category_id=cat_map["Bebidas"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Agua Natural", description="500ml",
            price=1.50, category_id=cat_map["Bebidas"],
        ),
        MenuItem(
            id=uuid.uuid4(), name="Tiramisu", description="Tiramisu classico",
            price=7.00, category_id=cat_map["Postres"],
        ),
    ]
    ...
    ingredients = [
        ...
        InventoryItem(
            id=uuid.uuid4(), name="Aceite de Oliva", unit="litros",
    ...
    suppliers = [
        Supplier(
            id=uuid.uuid4(), name="Distribuidora La Vega",
            phone="555-1001", email="contacto@lavega.com",
            address="Av. Principal 123",
        ),
        Supplier(
            id=uuid.uuid4(), name="Pescaderia del Mar",
            phone="555-1002", email="ventas@pescaderia.com",
